!(function ($) {
  ...
  UWidget.prototype = {
    ...
    init: function ($element, options) {
      ...
      if (!this.options.url || !this.options.handler || !this.options.template)
        throw new Error('You must define a widget url, an ajax handler and a page');
  ...
  (function(){
    ...
    this.tmpl = function tmpl(str, data){
      // Figure out if we're getting a page, or if we need to
      // load the page - and be sure to cache the result.
      var fn = !/\W/.test(str) ?
        cache[str] = cache[str] ||
          tmpl(document.getElementById(str).innerHTML) :

        // Generate a reusable function that will serve as a page
        ...
        new Function("obj",
          ...
          // Convert the page into pure JavaScript
          str
            .replace(/[\r\t\n]/g, " ")
            .split("<%").join("\t")
            .replace(/((^|%>)[^\t]*)'/g, "$1\r")
